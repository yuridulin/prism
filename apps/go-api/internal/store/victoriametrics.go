...
func (s *VictoriaMetrics) waitSamples(ctx context.Context, samples []model.Sample) error {
	if len(samples) == 0 {
		return nil
	}
	minTS := samples[0].TS.UTC()
	maxTS := samples[0].TS.UTC()
	...
	for {
		got, err := s.countExport(ctx, tagIDs, minTS.Add(-vmLookback), maxTS.Add(time.Minute))
		if err != nil {
			return err
		}
		if got >= want {
		...
		if time.Now().After(deadline) {
			return fmt.Errorf("vm: wrote %d samples but only %d visible", want, got)
		}
		select {
		case <-ctx.Done():
			return ctx.Err()
...
func (s *VictoriaMetrics) countExport(ctx context.Context, tagIDs []uint32, start, end time.Time) (int, error) {
	if len(tagIDs) == 0 {
		return 0, nil
	}
	params := url.Values{}
	...
	if resp.StatusCode >= 300 {
		body, _ := io.ReadAll(io.LimitReader(resp.Body, 2048))
		return 0, fmt.Errorf("vm export status %d: %s", resp.StatusCode, body)
	}
